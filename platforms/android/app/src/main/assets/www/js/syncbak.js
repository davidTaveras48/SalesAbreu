...
start(function () {
  ...
  var sync_cliente = function (vendedor) {
    return new Promise(function (ok, fail) {
      $.ajax(
        { url: http_server + "/sync_cliente", data: { vendedor: user.id } },
      )
        .done(function (r) {
  ...
  var sync_precios = function (x) {
    return new Promise(function (ok, fail) {
      $.ajax({ url: http_server + "/sync_precios", data: x })
        .done(function (r) {
          ok(r);
        })
        .fail(function (e) {
  ...
  var xsync_precios = function () {
    ...
    function x() {
      sync_precios({ desde: count, hasta: hasta })
        .then((r) => {
          count = count + 1000;
          hasta = count + 1000;

          if (valid == true) db.request("delete from precios");
          ...
          if (!r.length) {
            ...
            xmerc.insert(r)
              ...
              .then(() => {
                $("#pinfo").html("Actualizando Precios...");
                x();
              })
              .catch((e) => {
                vm = true;
