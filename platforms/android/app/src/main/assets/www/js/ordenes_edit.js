...
start(function(){
    ...
    b_merc=function(x){
        ...
        return db.request(`
            select codigo, nombre
                ,precio1+( (precio1/case when excento=1 then 1.18 else 1 end) *${cliente.incremento}/100) as precio1 
                ,precio2+( (precio2/case when excento=1 then 1.18 else 1 end)*${cliente.incremento}/100) as precio2 
                ,precio3+( (precio3/case when excento=1 then 1.18 else 1 end)*${cliente.incremento}/100) as precio3 
           ...
           where 
            inactivo=0 and 
            (
                nombre like '%${x.search}%'
                or referencia like '%${x.search}%'
                or codigo like '%${x.search}%'
            )
    ...
    $(document).on('click','.precio',function(){
        ...
        if(isMobile.any){
            var rid=$(this).parent().parent().parent().parent().parent().attr('rid')
        }else{
        ...
        $('.cantidad',$(this).parent().parent().parent().parent().parent()).trigger('change')


        $('.tipo_precio',$(this).parent().parent().parent().parent().parent()).html(merc.tipo_precio)
        merc.precio=merc['precio'+merc.tipo_precio]
        $('.precio_mostrar',$(this).parent().parent().parent().parent().parent()).html(util.nformat(merc.precio*1.18))
        return false;
    ...
    $(document).on('change','.cantidad',function(){
        ...
        var x=productos[merc.codigo]=productos[merc.codigo]||{precio:parseFloat(merc.precio),tipo_precio:parseInt(merc.tipo_precio)}
        ...
        x.valor=x.cantidad*parseFloat(merc.precio)
        if(!merc.excento)x.itbis=x.valor*.18
        else x.itbis=0
        ...
        $(this).parent().parent().children('.valor').html(util.nformat(x.valor,2))
    ...
    if (isMobile.any) {
        detalle=$('#detalle_movil').search('b_merc',{limit:10,order:'nombre'})    
    } else{
    ...
    detalle.on('request',function(r){
        ...
        for(var i=0;i<r.length;i++){
  	     ...
  	     if(productos[merc.codigo]){
            ...
            merc.iprecio=(merc.precio*1.18).toFixed(2)
        }
    })
